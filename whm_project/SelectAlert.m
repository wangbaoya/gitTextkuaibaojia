...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = wBlackColor;
...
- (void)initUI
{
    if (HANGGAO*_titles.count>=wScreenH-60)
    {
       self.alertView.frame = CGRectMake(15,30, wScreenW-30, wScreenH-60);
    }else
    {
       
       self.alertView.frame = CGRectMake(15,(wScreenH-HANGGAO*_titles.count)/2, wScreenW-30, HANGGAO*_titles.count+60);
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (!mylab)
    {
        mylab = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, wScreenW,HANGGAO)];
        mylab.font = Font(20);
        mylab.backgroundColor = wBaseColor;
        
        mylab.text = [NSString stringWithFormat:@"    请选择"];
     }
