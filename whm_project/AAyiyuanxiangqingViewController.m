//
//  AAyiyuanxiangqingViewController.m
//  whm_project
//
...
@interface AAyiyuanxiangqingViewController ()<UITableViewDelegate,UITableViewDataSource,BMKMapViewDelegate,BMKLocationServiceDelegate>
{
    UITableView * myTab;
    NSArray * lArr;
    NSArray * rArr;
...
@property(nonatomic,strong)NSMutableArray * gongsiArr;
...
@implementation AAyiyuanxiangqingViewController
...
- (void)viewDidLoad
{
    ...
    _gongsiArr = [NSMutableArray array];
    
    [_gongsiArr removeAllObjects];
    
    
    if (_gongsimingzi==666)
    {
                   //        mystr = [NSString stringWithFormat:@"%@",com.cname];
        [_gongsiArr addObject:_myDataModel.cname?_myDataModel.cname:@""];
     
 
    }else
    {
        for (ComModel * com in _myDataModel.com)
        {
            //        mystr = [NSString stringWithFormat:@"%@",com.cname];
            [_gongsiArr addObject:com.cname];
        }
    ...
    self.title = @"医院详情";
    lArr = @[@"医院等级",@"医院属性",@"医院性质",@"医保定点"];
    rArr = @[_myDataModel.level?_myDataModel.level:@"暂无信息",_myDataModel.autho?_myDataModel.autho:@"暂无信息",_myDataModel.nature.length>=1?_myDataModel.nature:@"暂无信息",[_myDataModel.desi isEqualToString:@"1"]?@"定点":@"非定点"];
    [self creatLeftTtem];
    [self creatUI];
}
...
-(void)creatUI
{
    ...
    UIView * upView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, wScreenW, 80)];
    ...
    [_imgBut sd_setImageWithURL:[NSURL URLWithString:_myDataModel.img] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"city"]];
    ...
    self.titLaber.textColor = wBlackColor;
     self.titLaber.text = _myDataModel.name.length > 1?_myDataModel.name:@"暂无公司";
...
self.mapImg.image = [UIImage iconWithInfo:TBCityIconInfoMake(@"\U0000e60f", 25, Wqingse)];
    ...
    self.telBut.frame = CGRectMake(CGRectGetMaxX(self.telImg.frame)+5, CGRectGetMaxY(self.addressLaber.frame),wScreenW-CGRectGetMaxX(self.telImg.frame)-25-10-10 ,lithhh);
    [self.telBut setTitleColor:DianhuaColor forState:UIControlStateNormal];
    self.telBut.titleLabel.font = Font(12);
...
//    [self.telBut setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 50)];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        AAyiyuanthreeTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"threecell" forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.downLab.numberOfLines = 0;

        if (indexPath.section==2)
        {
            cell.upLab.text = @"医院信息";
            cell.downLab.text = _myDataModel.info.length>=1?_myDataModel.info:@"暂无医院信息";
            
        }else if (indexPath.section==3)
        {
            cell.upLab.text = @"重点科室";
            cell.downLab.text = _myDataModel.depa.length>=1?_myDataModel.depa:@"暂无重点科室介绍";

        }else if(indexPath.section ==4)
        {
            cell.upLab.text = @"就诊指南";
            cell.downLab.text = _myDataModel.guide.length>=1?_myDataModel.guide:@"暂无就诊指南";
        }else
        {
            
            
            NSString * str = [_gongsiArr componentsJoinedByString:@","];
            cell.upLab.text = @"公司列表";
            cell.downLab.text = str.length>=1?str:@"暂无公司";
