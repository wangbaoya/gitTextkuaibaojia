...
@interface WbyJIgouViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,IFlyRecognizerViewDelegate>
{
...
@implementation WbyJIgouViewController
...
-(void)requestBaoXianGongSi:(NSString*)mystr
{
    NSMutableDictionary * dic = [NSMutableDictionary dictionary];
    
    [dic setObject:mystr forKey:@"keyword"];
    
    [dic setObject:_type?_type:@"" forKey:@"com_id"];
    
    [WBYRequest wbyPostRequestDataUrl:@"get_orgs" addParameters:dic success:^(WBYReqModel *model)
     {
         ...
         if ([model.err isEqualToString:TURE])
         {
             ...
             [myarr addObjectsFromArray:model.data];
...
-(void)aaarequestBaoXianGongSi:(NSString*)mystr
{
    NSMutableDictionary * dic = [NSMutableDictionary dictionary];
...
-(void)creatmyTab
{
    ...
    UIView * myview= [[UIView alloc] initWithFrame:CGRectMake(0, 0, wScreenW, 10)];
    ...
    gongsiTab.tableHeaderView = myview;
...
//    
//    ary = [ZYPinYinSearch searchWithOriginalArray:myarr andSearchText:searchText andSearchByPropertyName:@"name"];
//    
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if (_searchBar.text.length>=1)
    {
        ...
        [self requestBaoXianGongSi:_searchBar.text];
        
    }else
    {
        
        [WBYRequest showMessage:@"请输入搜索信息"];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DataModel * model = myarr[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.myGongsi(model.id?model.id:@"",model.name);
